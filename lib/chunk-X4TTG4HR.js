...
var JimengClient = class extends CreditService {
  ...
  async generateAuthorizationAndHeader(accessKeyID, secretAccessKey, sessionToken, region, service, requestMethod, requestParams, requestBody = {}) {
    return new Promise((resolve) => {
      ...
      if (Object.keys(requestBody).length > 0) {
        requestHeaders["X-Amz-Content-Sha256"] = crypto2.createHash("sha256").update(JSON.stringify(requestBody)).digest("hex");
      }
  ...
  addHeaders(amzDate, sessionToken, requestBody) {
    ...
    if (Object.keys(requestBody).length > 0) {
      headers["X-Amz-Content-Sha256"] = crypto2.createHash("sha256").update(JSON.stringify(requestBody)).digest("hex");
    }
    return headers;
  }
  ...
  canonicalString(requestMethod, requestParams, requestHeaders, requestBody) {
    ...
    const canonicalStringArr = [
      ...
      crypto2.createHash("sha256").update(body).digest("hex")
    ];
  ...
  signature(secretAccessKey, amzDate, region, service, requestMethod, requestParams, requestHeaders, requestBody) {
    ...
    const kDate = crypto2.createHmac("sha256", "AWS4" + secretAccessKey).update(amzDay).digest();
    ...
    const signingKey = crypto2.createHmac("sha256", kService).update("aws4_request").digest();
    const stringToSignArr = [
      "AWS4-HMAC-SHA256",
      amzDate,
      this.credentialString(amzDate, region, service),
      crypto2.createHash("sha256").update(
    ...
    return crypto2.createHmac("sha256", signingKey).update(stringToSign).digest("hex");
  }
  // ============== 视频后处理方法 ==============
  /**
...
//# sourceMappingURL=chunk-X4TTG4HR.js.map